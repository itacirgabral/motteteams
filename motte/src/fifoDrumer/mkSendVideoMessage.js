...
const mkSendVideoMessage = ({
  ...
  const { jid, quote, ondiskname, filename, mimetype, size, mark, caption, loop } = crumb
  const waittime = 300
  ...
  let videofile
  try {
    videofile = fs.readFileSync(path.join(process.cwd(), process.env.UPLOADFOLDER, ondiskname))
  } catch (error) {
  ...
  if (videofile) {
    ...
    if (quote && !quotedmessage) {
      ...
      bakedBread = await seed.conn.sendMessage(jid, videofile, MessageType.video, {
        mimetype: loop ? Mimetype.gif : mimetype,
        filename,
...
module.exports = mkSendVideoMessage
